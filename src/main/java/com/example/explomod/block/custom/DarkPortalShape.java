...
public class DarkPortalShape {
    private static final BlockBehaviour.StatePredicate FRAME = (state, level, pos) -> state.is(Blocks.DARK_PRISMARINE);
    ...
    public static Optional<DarkPortalShape> findEmptyAetherPortalShape(LevelAccessor level, BlockPos bottomLeft, Direction.Axis axis) {
        return findPortalShape(level, bottomLeft, (shape) -> shape.isValid() && shape.numPortalBlocks == 0, axis);
    }

    public static Optional<DarkPortalShape> findPortalShape(LevelAccessor level, BlockPos bottomLeft, Predicate<DarkPortalShape> predicate, Direction.Axis axis) {
        Optional<DarkPortalShape> optional = Optional.of(new DarkPortalShape(level, bottomLeft, axis)).filter(predicate);
        if (optional.isPresent()) {
            return optional;
        } else {
            Direction.Axis directionAxis = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
            return Optional.of(new DarkPortalShape(level, bottomLeft, directionAxis)).filter(predicate);
        }
    }

    public DarkPortalShape(LevelAccessor level, @org.jetbrains.annotations.Nullable BlockPos bottomLeft, Direction.Axis axis) {
        this.level = level;
        this.axis = axis;
    ...
    private int getDistanceUntilTop(BlockPos.MutableBlockPos mutablePos) {
        for (int i = 0; i < 21; ++i) {
            ...
            for (int j = 0; j < this.width; ++j) {
                ...
                if (blockState.is(ExploMod.DARK_PORTAL.get())) {
    ...
    private static boolean isEmpty(BlockState state) {
        return state.isAir() || state.is(Blocks.WATER) || state.is(ExploMod.DARK_PORTAL.get());
    }
    ...
    public void createPortalBlocks() {
        BlockState blockState = ExploMod.DARK_PORTAL.get().defaultBlockState().setValue(NetherPortalBlock.AXIS, this.axis);
        assert this.bottomLeft != null;
